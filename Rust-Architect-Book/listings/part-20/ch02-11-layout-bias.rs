// verify: release ok
// Layout bias: listing ch02-09 with ONE change. The two sum closures are defined in the opposite order, so the
// compiled loops land at different addresses. The instructions are the same (compare the asm of ch02-09);
// the timing of iter().sum() is not.
