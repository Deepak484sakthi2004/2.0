...
// verify: release ok
// Why Arc's drop is `fetch_sub(1, Release)` + `fence(Acquire)`: every thread's last use of the data must
// happen-before the free, and the fence lets only the LAST dropper pay for the Acquire.
use std::ops::Deref;
use std::ptr::NonNull;
