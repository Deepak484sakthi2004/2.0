...
// BROKEN on purpose: extend_from_slice publishes the new len BEFORE the clones are written.
// A panicking Clone (safe code!) leaves uninitialized slots inside [0, len); dropping the vector
// then drops garbage. Core identical to ch04-01-myvec-core.rs.
...
impl<T: Clone> MyVec<T> {
    pub fn extend_from_slice(&mut self, items: &[T]) {
        ...
        self.len += items.len(); // BUG: published before the slots are initialized
        for (i, item) in items.iter().enumerate() {
            // "SAFETY": reserved above. (True for the write, false for the len above.)
            unsafe { self.ptr().add(old_len + i).write(item.clone()) };
        }
    }
}
...
fn main() {
    ...
    println!("caught panic: {}, len now {}", result.is_err(), v.len());
} // v drops 4 slots; slots 2 and 3 were never written
