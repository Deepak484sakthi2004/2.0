...
// The push fast path of MyVec, next to std's Vec::push. Core identical to ch04-01-myvec-core.rs.
...
#[inline(never)]
pub fn push_mine(v: &mut MyVec<u64>, x: u64) {
    v.push(x)
}

#[inline(never)]
pub fn push_std(v: &mut Vec<u64>, x: u64) {
    v.push(x)
}

#[inline(never)]
pub fn sum_mine(v: &MyVec<u64>) -> u64 {
    v.iter().sum()
}
