...
// MyVec<T>, step 2: insert and remove shift the tail with ptr::copy (memmove).
// The core (RawVec, push, pop, Drop, Deref) is identical to ch04-01-myvec-core.rs.
...
impl<T> MyVec<T> {
    pub fn insert(&mut self, index: usize, value: T) {
        ...
        // SAFETY: len < cap, so [index, len] fits in the buffer. ptr::copy allows overlap (memmove).
        // Nothing between the copy and the write can panic, so no slot is ever exposed twice.
        unsafe {
            let p = self.ptr().add(index);
            ptr::copy(p, p.add(1), self.len - index); // shift [index, len) right by one
            p.write(value);
        }
        self.len += 1;
    }
    ...
    pub fn remove(&mut self, index: usize) -> T {
        ...
        // SAFETY: index < len. The value is moved out, the tail [index+1, old_len) slides left over it,
        // and len shrinks by one, so the stale bit-copy left in slot old_len-1 is outside [0, len).
        unsafe {
            self.len -= 1;
            let p = self.ptr().add(index);
            let out = p.read();
...
fn main() {
    ...
    v.insert(2, "c".to_string());
    v.insert(0, "start".to_string());
    let removed = v.remove(1);
    println!("removed={removed:?} now={:?} len={}", &v[..], v.len());
}
