...
// Drop check: because MyVec<T> implements Drop, the compiler must assume MyVec::drop may use the
// T's inside, so every borrow stored in the vector must outlive the vector. std's Vec opts out of
// that assumption with the unstable `#[may_dangle]` (Chapter 15.3); see ch04-12-dropck-std.rs.
...
fn main() {
    ...
    let owner = String::from("alice"); // declared after `names`, so dropped BEFORE it
    names.push(&owner);
    println!("{}", names.len());
}
