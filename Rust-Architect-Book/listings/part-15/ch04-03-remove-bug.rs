...
// BROKEN on purpose: `remove` forgets to shrink `len`. The tail slides left, but the last slot still
// counts as initialized, so the last element now exists twice. Dropping the vector drops it twice.
...
impl<T> MyVec<T> {
    pub fn remove(&mut self, index: usize) -> T {
        ...
        // "SAFETY": index < len.  (WRONG: the claim below about [0, len) is no longer true afterwards.)
        unsafe {
            let p = self.ptr().add(index);
            let out = p.read();
            ptr::copy(p.add(1), p, self.len - index - 1);
            // BUG: missing `self.len -= 1;`
            out
        }
    }
}
...
fn main() {
    ...
    println!("removed={removed:?} now={:?}", &v[..]); // ["b", "c", "c"]: the same String twice
} // v drops "c" twice: the same value is dropped twice (Miri reports it)
