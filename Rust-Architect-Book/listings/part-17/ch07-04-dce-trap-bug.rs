// verify: debug panic guarded
// Listing 17.7-4 (debugging exercise): listing 17.7-1 with ONE change in `dce`: instructions that may
// trap are no longer treated as live roots. Everything else is unchanged.
// Original header of 17.7-1: an optimizer. Ore source -> CFG (17.6-1) -> SSA (17.6-2) -> four passes:
...
fn dce(f: &mut SsaFn) -> usize {
    ...
    for b in &f.blocks {
        ...
        for _i in &b.insts {
            // (the bug: trapping instructions are no longer roots)
        }
    }
    ...
    for b in f.blocks.iter_mut() {
        ...
        b.insts.retain(|i| matches!(i, Inst::Call { .. }) || match i {
