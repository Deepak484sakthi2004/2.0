// verify: debug panic K=
// Listing 17.8-3 (debugging exercise): listing 17.8-1 with ONE change in `intervals`: a value's interval
// runs from its first to its last TEXTUAL appearance (live-out sets are ignored). Everything else is unchanged.
// Original header of 17.8-1: the back end, end to end. Three-address code with unlimited virtual registers ->
...
fn intervals(code: &[Tac], _live_out: &[Vec<u32>]) -> Vec<(u32, usize, usize)> {
    ...
    for (i, t) in code.iter().enumerate() {
        ...
        uses.into_iter().chain(def).for_each(|v| touch(v, i)); // the bug: live_out[i] is ignored
    }
