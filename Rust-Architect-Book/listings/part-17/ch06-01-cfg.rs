// verify: debug ok
// Listing 17.6-1: lowering Ore to a control-flow graph of basic blocks (three-address code),
// then the analyses every optimizer starts with: dominators (Cooper, Harvey & Kennedy 2001),
// dominance frontiers, and back edges (loops). An interpreter runs the IR to check the lowering.
...
fn classify(a: int, b: int) -> int {
    ...
    let fns: HashMap<String, FnIr> = items.iter().map(|f| (f.name.clone(), lower_fn(f))).collect();

    for name in ["sum_to", "classify"] {
        let f = &fns[name];
        print_fn(f);
        let idom = dominators(f);
        let df = dominance_frontiers(f, &idom);
        let idoms: Vec<String> = (1..f.blocks.len()).map(|b| format!("bb{b}:bb{}", idom[b])).collect();
        println!("  idom      {}", idoms.join(" "));
        let dfs: Vec<String> = df.iter().enumerate().filter(|(_, d)| !d.is_empty())
            .map(|(b, d)| format!("DF(bb{b})={{{}}}", d.iter().map(|x| format!("bb{x}")).collect::<Vec<_>>().join(","))).collect();
        println!("  frontiers {}", dfs.join(" "));
        for (b, bb) in f.blocks.iter().enumerate() {
            for s in bb.term.as_ref().unwrap().succs() {
                if dominates(&idom, s, b) {
                    println!("  back edge bb{b} -> bb{s}: bb{s} is a loop header");
                }
            }
        }
        println!();
    ...
    let mut steps = 0;
    for (name, args) in [("sum_to", vec![10]), ("fib", vec![10]), ("classify", vec![0, 5]), ("classify", vec![2, 9]), ("classify", vec![3, 5])] {
        println!("{name}{args:?} = {:?}", run(&fns, name, &args, &mut steps));
    }
    println!("({steps} IR instructions executed; classify(0, 5) never divided by zero)");
}
